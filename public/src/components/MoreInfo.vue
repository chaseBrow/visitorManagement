	...
	<v-container class="ma-0 pa-0">
					...
					<v-row>
						<v-col cols="4">
							<v-text-field
								...
								label="First Name"
								v-bind="{ rounded: !edit, readonly: !edit, outlined: edit }"
		...
		<v-dialog width="400px" v-model="dialogDel">
			<v-card>
				...
				<v-card-text>
					You will no longer be able to add visit records for this visitor. All
					previous visits will still be avaliable on the Visit History tab.
				</v-card-text>
				<div class="d-flex justify-center">
					<v-btn
						...
						color="info"
						outlined
						v-on:click="cancelDel()"
					>
						Cancel
					</v-btn>
					<v-btn
						class="warning mb-6"
						v-on:click="confirmDel()"
						v-bind="{ loading: delLoading }"
					>
						Delete
					</v-btn>
				</div>
	...
	data() {
		return {
			...
			visitor: {
				...
				emailRules: [v => !!v || "Email is a required field."],
